# ¡Hola! Soy Emir Segovia 👋

Una persona proactiva, con sólidas habilidades de comunicación y que entiende la importancia del trabajo en equipo. Me motiva aportar valor en cada proyecto, brindando lo necesario para alcanzar los objetivos de manera eficiente. Disfruto aprender continuamente y seguir creciendo, lo que me permite adaptarme con facilidad y dar siempre lo mejor de mí ante cada nuevo desafío.

## Sobre mí
- 🎓 Técnico Universitario en Programación [UTN FRT] (2022 - 2024)
- ⌛🎓 Estudiante de Ingeniería en Sistemas de Información [UTN FRT]
- 💼 QA Analyst con 3+ años de experiencia
- 📍 Tucumán, Argentina
- 🌐 Español | Inglés (B1)
- 📧 emirvalles90@gmail.com
- 📱 +54 3865 761910

## Experiencia Profesional

### 🧪 Doors Tickets — QA Analyst
📍 Remoto | 📅 08/2023 - Actualidad

Plataforma digital para la gestión integral de eventos, ventas y control de accesos.

• Responsable del diseño y ejecución de casos de prueba manuales para validar funcionalidades según criterios de aceptación, asegurando la correcta implementación de flujos críticos como accesos por DNI/QR, reportes en tiempo real y operaciones de venta.
• Automatización de pruebas funcionales y de regresión utilizando Playwright, así como mantenimiento y actualización del código de automatización para acompañar la evolución del producto.
• Trabajo colaborativo con el equipo de desarrollo para el reporte, análisis y seguimiento de defectos, ejecución de pruebas smoke y regresión en cada iteración, y gestión de tablero Kanban para garantizar un flujo de trabajo ordenado, trazable y eficiente.

### 🧩 Softnik — QA Analyst (Part-time)
📍 Remoto | 📅 10/2025 - Actualidad

Plataforma cloud orientada a la gestión integral de recursos humanos, control de asistencia y administración de empleados.

• Encargado del diseño y ejecución de pruebas manuales sobre funcionalidades clave como control horario, gestión de usuarios, reportes personalizados y módulos adicionales del ecosistema, validando reglas de negocio y criterios de aceptación.
• Trabajo colaborativo con el equipo de desarrollo para el reporte, análisis y seguimiento de defectos, ejecución de pruebas smoke y regresión en cada iteración.

## Certificaciones
- 🏅 **Quality Control Automation** - Globant University | Egg (2024)
- 🏅 **Quality Assurance** - QARMY | Underc0de (2023)
- 🏅 **Accredited Software Testing Fundamentals (ASTFC)** - AICS (2023)
- 🏅 **API Fundamentals Student Expert** - Postman (2023)
- 🏅 **SCRUM Fundamentals Certified (SFC)** - SCRUM Study (2023)

## Tecnologías y herramientas
- **Testing Tools**: Playwright, Cypress, Postman
- **Project Management**: Jira, Trello
- **Lenguajes**: JavaScript, TypeScript, Java, SQL
- **DevOps & Version Control**: Git, GitHub Actions

## Estemos en contacto!

- 💼 [LinkedIn](https://www.linkedin.com/in/emir-segovia-74b392113)
- 📧 [Correo](mailto:emirvalles90@gmail.com)
- 📁 [Portafolio](https://emir-portfolio-five.vercel.app/)

Estoy abierto a colaborar en proyectos de QA Automation. Si tienes alguna propuesta o sugerencia, ¡no dudes en contactarme!

# Apache90
